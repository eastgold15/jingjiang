...
**视觉效果：** 居中对称，超大加粗白色主标题 + 浅灰副标题 + 浅紫细水平分割线 + 页脚左右分栏。
...
**视觉效果：** 与 `cover` 相同的居中风格，但内容区域略有调整。

**适用场景：** 演示文稿的第二页或章节开头，用于简要介绍背景、目录或整体概况。与封面风格统一，形成过渡。
...
在 Slidev 的 `cover` 布局中，可以直接在 Markdown 中使用 HTML/Vue 模板语法来排版。推荐做法如下：

### 方法一：直接写在 Markdown 内容区
...
### 方法二：使用 slidev 的 frontmatter 插值（更推荐）

```yaml
---
layout: cover
title: 专业设置调整方案
subtitle: 专业申报与优化论证报告
---
...
**要点说明：**
- 单位名称写在页脚左侧，日期写在右侧，利用 Flexbox 左右排布
- 使用 `pt-12` 将页脚推到分割线下方合适位置
- 文字颜色用 `text-gray-300` 或自定义浅灰色，与封面正式风格统一
- 如需日期自动更新，可借助 Vue 语法 `{{ new Date().toLocaleDateString('zh-CN', { year: 'numeric', month: 'long' }) }}`

---

## 四、如何放大查看 Mermaid 图表

使用 `<MermaidView>` 组件包装 `mermaid` 代码块即可获得完整的缩放交互体验。
...
| 操作方式 | 细节 |
|----------|------|
| **滚轮缩放** | 以鼠标指针所在位置为缩放中心，滚动滚轮即可放大/缩小 |
| **拖拽平移** | 按住鼠标左键拖拽，图表跟随移动 |
| **工具栏按钮** | 右上角工具栏：`−` 缩小，`+` 放大，`⟲` 重置为 100% |
| **缩放范围** | 25% ~ 300%，超出范围自动 clamp |

### 为什么比 Slidev 原生 Mermaid 更好

Slidev 内置的 ```` ```mermaid ```` 虽然能渲染图表，但图表超出页面时无法缩放查看。而 `MermaidView` 提供了完整的平移、缩放、重置能力，适合展示架构图等复杂图表。
...
| 正文内容 | `circle-tr-bl` | Card, ScrollView |
| 数据对比 | 任意内容布局 | Card（多栏并列） |
| 系统架构展示 | 任意内容布局 | MermaidView |
| 长文本/代码 | 任意内容布局 | ScrollView |
| 底部汇总 | 任意内容布局 | Card（size=`full`） |
